...
if %ERRORLEVEL%==0 (
    echo %_ERROR_LABEL% Failed to copy ADW symbol files ^("%_ADWM2_HOME%\winamd64sym"^) 1>&2
    set _EXITCODE=1
    goto :eof
)
...
(
    ...
    echo -ENTRY:%_APP_NAME%_$InitCode
) > "%__LINKER_OPTS_FILE%"
...
if %__N%==1 ( set __N_FILES=%__N% Modula-2 implemenation module
...
@rem    echo %_ERROR_LABEL% Failed to list entries in "!_TARGET_FILE:%_ROOT_DIR%=!" 1>&2
@rem    set _EXITCODE=1
@rem    goto :eof
@rem )
