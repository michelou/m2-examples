...
goto args_loop
:args_done
set _STDERR_REDIRECT=1^>NUL
if %_DEBUG%==1 set _STDERR_REDIRECT=

if exist "%_SOURCE_DIR%\def-%_TOOLSET%" (
    set "_SOURCE_DEF_DIR=%_SOURCE_DIR%\def-%_TOOLSET%"
)
...
xcopy /i /q /y "%ADWM2_HOME%\Unicode\winamd64sym\*.sym" "%_TARGET_SYM_DIR%" 1>NUL
if exist "%_LIB_DIR%\*.sym" (
    if %_DEBUG%==1 echo %_DEBUG_LABEL% copy /y "%_LIB_DIR%\*.sym" "%_TARGET_SYM_DIR%\" 1>&2
    copy /y "%_LIB_DIR%\*.sym" "%_TARGET_SYM_DIR%\" 1>NUL
)
...
set __M2C_OPTS=-sym:!_TARGET_SYM_DIR:%_TARGET_DIR%\=!;!_TARGET_DEF_DIR:%_TARGET_DIR%\=!
...
:compile_adw_mod
pushd "%_TARGET_DIR%"
if %_DEBUG%==1 echo %_DEBUG_LABEL% @rem Current directory is: "%CD%" 1>&2
for /f "delims=" %%f in ('dir /s /b "%_TARGET_MOD_DIR%\*.mod" 2^>NUL') do (
    set "__MOD_FILE=%%f"
    set "__MOD_FILE=!__MOD_FILE:%_TARGET_DIR%\=!"
