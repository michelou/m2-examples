...
function Compile-ADW
{
    ...
    Add-Content -Path $LINKER_OPTS_FILE -Value '-SUBSYSTEM:CONSOLE'
    ...
    (Get-ChildItem -Path $TARGET_MOD_DIR -Include *.obj).FullName | ForEach-Object {
        Add-Content -Path $LINKER_OPTS_FILE -Value $_
    }
    ## object files of library depencencies
    (Get-ChildItem -Path $TARGET_BIN_DIR -Include *.obj).FullName | ForEach-Object {
        Add-Content -Path $LINKER_OPTS_FILE -Value $_
    }
...
function Run
{
    if (! (Test-Path -PathType Leaf -Path $TARGET_FILE)) {
        Write-Error "Modula-2 program ""$($TARGET_FILE.Replace($ROOT_DIR + $SEP, ''))"" not found"
        Cleanup 1
    }
