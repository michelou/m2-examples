...
function Compile
{
    ...
    if ($TOOLSET -eq 'adw') { Compile-ADW $ACTION_DEF }
    elseif ($TOOLSET -eq 'gnu') { Compile-GNU $ACTION_DEF }
    else { Compile-XDS $ACTION_DEF }
}
...
function Compile-ADW
{
    ...
    Write-Debug "Copy-Item -Path ($LIB_DIR + $SEP + '*.sym') | Copy-Item -Destination $TARGET_SYM_DIR"
...
function Compile-XDS
{
    ...
    Write-Debug "Get-ChildItem -Path ($LIB_DIR + $SEP + '*.sym') | Copy-Item -Destination $TARGET_BIN_DIR"
    Copy-Item -Path ($LIB_DIR + $SEP + '*.lib') -Destination $TARGET_BIN_DIR

    Write-Debug "Get-ChildItem -Path ($LIB_DIR + $SEP + '*.sym') -Destination $TARGET_SYM_DIR"
    Copy-Item -Path ($LIB_DIR + $SEP + '*.sym') -Destination $TARGET_SYM_DIR
    ...
    Write-Debug "Get-ChildItem -Path ($SOURCE_MOD_DIR + $SEP + '*.mod') -Destination $TARGET_MOD_DIR"
    Copy-Item -Path ($SOURCE_MOD_DIR + $SEP + '*.mod') -Destination $TARGET_MOD_DIR
