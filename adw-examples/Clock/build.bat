...
if "%__ARG:~0,1%"=="-" (
    ...
    @rem ) else if "%__ARG%"=="-xds" ( set _TOOLSET=xds
    ) else (
...
@rem echo     %__BEG_O%-adw%__END%         select ADW Modula-2 toolset
echo     %__BEG_O%-debug%__END%       print commands executed by this script
echo     %__BEG_O%-verbose%__END%     print progress messages
@rem echo     %__BEG_O%-xds%__END%         select XDS Modula-2 toolset ^(default^)
echo.
...
if exist "%_ADWM2_HOME%\winamd64sym\*.sym" (
    if %_DEBUG%==1 echo %_DEBUG_LABEL% xcopy /i /q /y "%_ADWM2_HOME%\winamd64sym\*.sym" "%_TARGET_SYM_DIR%" 1>&2
    xcopy /i /q /y "%_ADWM2_HOME%\winamd64sym\*.sym" "%_TARGET_SYM_DIR%\"
)
...
set __M2C_OPTS=-nowarn -sym:!_TARGET_SYM_DIR:%_ROOT_DIR%=!,!_TARGET_DEF_DIR:%_ROOT_DIR%=!
...
@rem implementation modules (without main program)
for /f "delims=" %%f in ('dir /s /b "%_TARGET_MOD_DIR%\*.mod" ^| findstr /v Clock.mod 2^>NUL') do (
    set "__MOD_FILE=%%f"
...
:compile_adw_main
for /f "delims=" %%f in ('dir /s /b "%_TARGET_MOD_DIR%\Clock.mod" 2^>NUL') do (
    set "__MOD_FILE=%%f"
    if %_DEBUG%==1 ( echo %_DEBUG_LABEL% %_M2C_CMD% %__M2C_OPTS% "!__MOD_FILE!" 1>&2
    ) else if %_VERBOSE%==1 ( echo Compile "!__MOD_FILE!" into directory "!_TARGET_MOD_DIR:%_ROOT_DIR%=!" 1>&2
    )
